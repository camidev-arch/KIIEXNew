...
const SectionThree = () => {
    return (
        ...
        <div className="section-three">
            <Row className='bg-row3 row-center main-row-three' >
                <Col span={16} style={{ paddingTop: '100px', marginBottom: '60px', maxWidth:'0 !important' }}>
                    <h2 className='title-three-one' style={{fontSize:'40px !important', textAlign:'center'}}>
                        <span style={{color:'#00A0E6'}}>Los beneficios</span> que tenemos para ti no los encontrarás en ningún otro lugar
                    </h2>
            ...
            <Row style={{ display:'flex', justifyContent:'center', paddingRight: '12%', paddingLeft: '12%', paddingBottom: '100px' }}>
                {CARDS.map((card) => {
                    return <Col md={12} xs={24} lg={10} sm={24} span={8} style={{ margin: '0px', marginRight:'-30px',marginLeft:'-30px',padding: '0px',display:'flex', alignItems:'center', justifyContent:'center',  }}> <ExchangeCard colorBody='white' colorText='#0F4DCC' clase={card.title == 'Compra y vende' ? 'cards1': undefined} description={card.description} title={card.title} icon={card.icon} /> </Col>
                })}
            </Row>
        </div>
        </>
